...
if __name__=="__main__":
    crop_meta_path = str(sys.argv[1])
    raw_data_path = str(sys.argv[2])
    raw_meta = json.load(open(crop_meta_path, 'r'))
    n_Pano = len(raw_meta)
    crop_num = int(sys.argv[3])
    pairs_num = int(sys.argv[4])
    pairs_meta={}
    pair_cnt = 0
    meta_out_dir = str(sys.argv[5])
    os.makedirs(meta_out_dir, exist_ok=True)
    
    for pano_name in tqdm(raw_meta.keys()):
        crops_dict = raw_meta[pano_name]
        pano_base_dir = os.path.join(raw_data_path, pano_name, "crops")
    ...
    np.save(meta_out_dir+'/sun_360.npy', pairs_meta, allow_pickle=True)